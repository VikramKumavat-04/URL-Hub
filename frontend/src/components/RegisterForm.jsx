...
export default function RegisterForm({ state }) {
  ...
  const update = (k, v) => { setForm((f) => ({ ...f, [k]: v })); setErrs((e) => ({ ...e, [k]: "" })); if (error) dispatch(clearError()); };
  ...
  const handleSubmit = async (ev) => {
    ...
    if (Object.keys(nextErrs).length) return setErrs(nextErrs);
    const res = await dispatch(register({ name: form.name.trim(), email: form.email.trim(), password: form.password }));
    if (register.fulfilled.match(res)) { setSuccess("Account created! Redirecting..."); window.setTimeout(() => state(true), 1200); }
  };

  return (
    <AuthShell heading="Create account" sub="Start managing secure, trackable short links." toggleLabel="Already have an account?" toggleAction="Sign in" onToggle={() => state(true)}>
      {success && <Banner type="success">{success}</Banner>}
      {error && <Banner type="error">{error}</Banner>}
      <form onSubmit={handleSubmit} style={{ display: "grid", gap: 14 }}>
        <Field label="Full name" value={form.name} onChange={(v) => update("name", v)} placeholder="Alex Johnson" error={errs.name} disabled={loading} />
        <Field label="Email address" type="email" value={form.email} onChange={(v) => update("email", v)} placeholder="you@example.com" error={errs.email} disabled={loading} />
        <FieldPwd label="Password" value={form.password} show={showPwd} onToggle={() => setShowPwd(s => !s)} onChange={(v) => update("password", v)} placeholder="6+ characters" error={errs.password} disabled={loading} />
        <Field label="Confirm password" type="password" value={form.confirm} onChange={(v) => update("confirm", v)} placeholder="Same password again" error={errs.confirm} disabled={loading} />
